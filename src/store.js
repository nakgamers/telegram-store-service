...
const demoProducts = [
  { id: 'api-key-1d', name: 'API KEY 1 HARI', type: 'api_access', price: 5000, durationDays: 1, description: 'API key 9Router aktif selama 1 hari.', delivery: 'api_key' },
  { id: 'api-key-3d', name: 'API KEY 3 HARI', type: 'api_access', price: 10000, durationDays: 3, description: 'API key 9Router aktif selama 3 hari.', delivery: 'api_key' },
  { id: 'api-key-7d', name: 'API KEY 7 HARI', type: 'api_access', price: 15000, durationDays: 7, description: 'API key 9Router aktif selama 7 hari.', delivery: 'api_key' },
];
...
export async function initStore() {
  ...
  const schema = databaseDriver === 'mysql' ? `
    CREATE TABLE IF NOT EXISTS store_products (id VARCHAR(100) PRIMARY KEY, name VARCHAR(255) NOT NULL, type VARCHAR(50) NOT NULL, price BIGINT NOT NULL, duration_days INT NOT NULL DEFAULT 1, description TEXT NOT NULL, delivery VARCHAR(50) NOT NULL DEFAULT 'manual', active TINYINT(1) NOT NULL DEFAULT 1, created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP);
  ...
  ` : `
    CREATE TABLE IF NOT EXISTS store_products (id TEXT PRIMARY KEY, name TEXT NOT NULL, type TEXT NOT NULL, price BIGINT NOT NULL, duration_days INTEGER NOT NULL DEFAULT 1, description TEXT NOT NULL DEFAULT '', delivery TEXT NOT NULL DEFAULT 'manual', active BOOLEAN NOT NULL DEFAULT TRUE, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW());
  ...
  if (databaseDriver === 'mysql') await dbQuery('ALTER TABLE store_products ADD COLUMN IF NOT EXISTS duration_days INT NOT NULL DEFAULT 1 AFTER price');
  for (const product of demoProducts) {
    const insert = databaseDriver === 'mysql'
      ? 'INSERT IGNORE INTO store_products (id,name,type,price,duration_days,description,delivery) VALUES (?,?,?,?,?,?,?)'
      : 'INSERT INTO store_products (id,name,type,price,duration_days,description,delivery) VALUES ($1,$2,$3,$4,$5,$6,$7) ON CONFLICT (id) DO UPDATE SET name=EXCLUDED.name, type=EXCLUDED.type, price=EXCLUDED.price, duration_days=EXCLUDED.duration_days, description=EXCLUDED.description, delivery=EXCLUDED.delivery';
    await dbQuery(insert, [product.id, product.name, product.type, product.price, product.durationDays, product.description, product.delivery]);
  }
...
export async function listProducts() {
  ...
  const { rows } = await dbQuery('SELECT id,name,type,price,duration_days,description,delivery FROM store_products WHERE active = TRUE ORDER BY name');
  return rows.map((row) => ({ ...row, durationDays: Number(row.duration_days ?? row.durationDays ?? 1) }));
}
...
export async function createOrder({ telegramId, productId, quantity = 1 }) {
  ...
  const order = { id, telegramId: Number(telegramId), productId: product.id, productName: product.name, productType: product.type, durationDays: Number(product.durationDays || 1), quantity, total, status: 'pending_payment', delivery: product.delivery, expiresAt: expiresAt.toISOString() };
